...
func (h *HomeController) RegisterRoutes(router *gin.RouterGroup) {
	...
	{
		homeGroup.GET("/books", config.RedisCacheHandler(h.ListHomeBooks, time.Minute))
		homeGroup.GET("/friend_Link/list", config.RedisCacheHandler(h.ListHomeFriendLinks, time.Hour*2))
	}
}
